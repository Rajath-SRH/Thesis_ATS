...
def get_random_pair(session_id):
    ...
    db = client.ATS
    cur_cv = db.cv_corpus.find({"session_id": session_id})
    cur_jd = db.jd_corpus.find({"session_id": session_id})
...
def getUser(cv_mongo_object_id):
    client = pymongo.MongoClient("localhost", 27017)
    db = client.ATS
    cv_cur = db.cv_struct.find({"_id": ObjectId(cv_mongo_object_id)})
...
def normalized_score(results):
    ...
    for item in results:
      if not math.isnan(item[8]):
        if item[8] > 2:
          tmp[item[1]].append(int(item[8]))



    return [{'filename':getUser(k), 'result':round(statistics.mean(v)*20, 2)} for k,v in tmp.items()]
